...
"""SPADE: memory-bank anomaly detection by k-nearest-neighbour retrieval.

Included to test whether the generalisation gap reported for PatchCore-style
detectors is a property of the memory-bank *paradigm* or only of PatchCore's
particular instantiation of it. SPADE keeps the memory-bank idea but changes
essentially everything else about how it is used:

    PatchCore                          SPADE
    ---------------------------------  -----------------------------------
    stores a coreset of patch vectors  stores one descriptor per image
    1% greedy farthest-point subset    keeps every reference image
...
The backbone (WideResNet-50, ImageNet weights) and the layers used are identical
to PatchCore's, so any difference in behaviour is attributable to the scoring
rule rather than to the representation.
...
class SPADE(AnomalyMethod):
    ...
    def __init__(
        self,
        # SPADE retrieves whole images, for which its authors use deep semantic
        # features; restricting it to PatchCore's layer2+layer3 costs it 0.10
        # AUROC in-distribution (0.567 vs 0.664 on MVTec), which would
        # understate it. layer4 is therefore included. The backbone is
        # still identical to PatchCore's, so the representation family is held
        # fixed even though the depth each method draws on is its own.
    ...
    def _descriptors(self, paths: list[Path], desc: str) -> np.ndarray:
        """One globally average-pooled descriptor per image, as SPADE retrieves
        whole reference images rather than individual patches."""
        out = []
    ...
    def fit(self, image_paths: list[Path], seed: int = 0) -> None:
        ...
        if len(paths) > self.max_train_images:
            # Same reference-image subsampling as PatchCore, so the two methods
            # see exactly the same normal images for a given seed.
